...
def actor_learner_thread(thread_id, env, num_actions, saver, q, input_directory):
    ...
    file = open('/media/youndukn/lastra/plants_data/{}_data_{}'.format(input_name[0].replace(input_directory, ""), thread_id), 'wb')
    ...
    while T < TMAX:
        ...
        while True:
            ...
            posibilities = random.randrange(6)
            if posibilities == 0:
...
def train(num_actions):
    ...
    for subDirectory in ['c{0:02}'.format(x) for x in range(5, 6)]:
        ...
        for t in actor_learner_threads:
            t.join()
    """
    for subDirectory in ['c{0:02}'.format(x) for x in range(4, 16)]:
