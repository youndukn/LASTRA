...
def actor_learner_thread(thread_id, env, session, graph_ops, num_actions,
    ...
    """
    s0 = graph_ops["s0"]
    q_values0 = graph_ops["q_values0"]
    a0 = graph_ops["a0"]
    y0 = graph_ops["y0"]
    grad_update0 = graph_ops["grad_update0"]
    """
    s1 = graph_ops["s1"]
    q_values1 = graph_ops["q_values1"]
    a1 = graph_ops["a1"]
    y1 = graph_ops["y1"]
    grad_update1 = graph_ops["grad_update1"]
    """
    s2 = graph_ops["s2"]
    q_values2 = graph_ops["q_values2"]
    a2 = graph_ops["a2"]
    y2 = graph_ops["y2"]
    grad_update2 = graph_ops["grad_update2"]
    ...
    y_batch2 = []
    r_batch = []
    append_data = TrainSet(s_batch_l0, [[1, ], ], y_batch2)
        ...
        if os.path.isfile('./data1/data_{}_{}'.format(thread_id, i)):
            ...
            try:
                while True:
                    ...
                    if i == 0:
                        y_batch_init = np.divide(train_set.total_reward, [10, ])
                    else:
                    ...
                    if len(y_batch_init)>1:
                        y_batch_init_temp = y_batch_init[start_index:]
                        a_batch_init_temp = a_batch_init[start_index:]
                        s_batch_init_temp = s_batch_init[start_index:]
                    else:
                        y_batch_init_temp = y_batch_init
    ...
    for i in range(0, 1):
        ...
        if os.path.isfile('./data1/data_{}'.format(thread_id)):
            ...
            print('Processing File ./data1/data_{}'.format(thread_id))
            t_init = 0
            try:
                while True:
                    ...
                    hello = 0
                    for node, current_r in enumerate(r_batch_init):
                        if isinstance(current_r, list) and node < len(r_batch_init)-1:
                            s_temp.append(s_batch_init[hello])
                            a_temp.append([1, ])
                            r_temp0.append(current_r[0])
                            r_temp2.append(current_r[4])
                            hello = hello+1

                    if len(s_temp)>0 and len(r_temp0)>0:
                        session.run(grad_update0, feed_dict={y0: r_temp0,
                                                             a0: a_temp,
                                                             s0: s_temp})
                        session.run(grad_update2, feed_dict={y2: r_temp2,
                                                             a2: a_temp,
                                                             s2: s_temp})
